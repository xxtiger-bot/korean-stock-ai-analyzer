...
韩国股票分析 Web 软件 MVP。项目面向韩国股票用户，提供真实行情、K 线、技术指标、AI 分析、自选股优先级和今日简报，帮助用户快速判断“今天先看什么、风险在哪里、需要观察哪些位置”。
...
- 今日机会雷达：基于韩国代表股票池的真实行情和 K 线计算触发信号。
- 热门股票：基于韩国代表股票池的真实成交量、成交额、波动率和数据完整度排序。
- 搜索：支持按韩国股票代码和韩文名称搜索，例如 `005930`、`삼성전자`。
- 股票详情页：真实当前价格、涨跌幅、成交量、市值、市场、数据来源。
- K 线图：真实 `data.go.kr` 日线数据。
- 技术指标：基于真实 K 线计算 MA5、MA20、MA60、RSI、MACD。
- AI 분석 리포트：基于真实行情上下文生成趋势、技术面、风险和观察点。
...
- `OPENAI_API_KEY`：存在时调用 OpenAI 生成报告；不存在时使用真实行情上下文生成本地 fallback 报告。
