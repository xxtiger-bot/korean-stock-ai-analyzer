  ...
  const trustItems = [
    {
      icon: BarChart3,
      title: "데이터 출처",
      body: "현재가·장중 시세는 한국투자증권(KIS) API를 기준으로 반영하고, 일별 종가·이동평균·RSI·MACD와 기준일 정보는 data.go.kr(공공데이터포털) 데이터로 계산합니다."
    },
    {
      icon: ShieldCheck,
      title: "로컬 모드 우선",
      body: "기본 모드는 로컬 우선으로 동작하며, 로그인하지 않으면 보유종목·알림·체크리스트 데이터가 브라우저에만 저장되고 서버로 자동 업로드되지 않습니다."
    },
    {
      icon: Target,
      title: "투자 참고 정보",
      body: "분석 결과는 투자 참고 정보이며, 최종 판단과 책임은 사용자에게 있습니다."
    }
  ] as const;
  const userVoices = [
    {
      role: "베타 사용자",
      comment: "아침에 브리핑과 TOP 3를 먼저 보면, 출근길에 오늘 점검할 종목 순서가 바로 정리됩니다."
    },
    {
      role: "개인 투자자",
      comment: "보유종목 카드에서 리스크 변화와 알림 근접을 같이 보니, 감정적으로 대응하지 않고 재평가 시점을 정하기 수월했습니다."
    },
    {
      role: "베타 참여자",
      comment: "처음에는 로그인 없이 로컬 모드로 가볍게 써보고, 필요할 때만 동기화를 켜서 사용 흐름이 자연스러웠습니다."
    }
  ] as const;
