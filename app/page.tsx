  ...
  const trustItems = [
    {
      icon: BarChart3,
      title: "데이터 출처",
      body: "현재가와 장중 가격 흐름은 한국투자증권(KIS) 시세 API를 기준으로 제공하며, 일별 종가·이동평균·RSI·MACD 등 기술지표와 기준일 데이터는 data.go.kr(공공데이터포털) 자료를 기준으로 산출합니다."
    },
    {
      icon: ShieldCheck,
      title: "로컬 모드 우선",
      body: "로그인 없이 사용하면 데이터는 사용자 브라우저에 우선 저장되며, 클라우드로 자동 전송되지 않습니다."
    },
    {
      icon: Target,
      title: "투자 참고 정보",
      body: "이 서비스는 매수/매도 추천이 아닌 투자 참고 정보 제공을 목적으로 하며, 최종 판단과 책임은 사용자에게 있습니다."
    }
  ] as const;
  const userVoices = [
    {
      role: "베타 사용자",
      comment: "출근 전에 오늘 시장 브리핑과 TOP 3만 먼저 확인해도, 어떤 종목부터 점검할지 바로 정리돼서 아침 준비 시간이 줄었습니다."
    },
    {
      role: "개인 투자자",
      comment: "보유종목 화면에서 리스크 변화와 알림 근접 상태를 같이 보니까, 감으로 판단하지 않고 재평가 시점을 더 차분하게 잡게 됐습니다."
    },
    {
      role: "베타 참여자",
      comment: "로그인 없이 로컬 모드로 바로 써볼 수 있어서 진입 장벽이 낮았고, 필요할 때만 로그인해 동기화를 켤 수 있어 편했습니다."
    }
  ] as const;
