...
function buildChecklistItem(diagnosis: PortfolioDiagnosis, conditions: UserAlertCondition[]): ChecklistItem {
  ...
  return {
    ...
    symbol: diagnosis.symbol,
    name: diagnosis.stockName || diagnosis.symbol,
    buyPrice: safeNumber(diagnosis.buyPrice),
    quantity: safeNumber(diagnosis.quantity),
    currentPrice: safeNumber(diagnosis.currentPrice),
    returnRate,
    statusTag,
  ...
  const watchlistItems = useMemo(() => {
    ...
    for (const symbol of safeWatchlistSymbols) {
      ...
      items.push({
        ...
        buyPrice: null,
        quantity: null,
        currentPrice: safeNumber(stock.price),
                ...
                {(variant === "home" ? riskChangesSorted.slice(0, 3) : riskChangesSorted).map((item) => (
                  <li key={`${item.symbol}-${item.direction}`} className="rounded-md border border-line bg-white px-3 py-2 dark:border-dark-line dark:bg-dark-panel">
                    ...
                    <p className="mt-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
                      어제 상태: {item.yesterdayStatus}
                    </p>
                    <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                      오늘 상태: {item.todayStatus}
                    </p>
                    <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                      변화 이유: {item.reason}
                    </p>
