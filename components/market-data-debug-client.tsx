  ...
  const handleReload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setReloadMessage(`재검사 실패: ${errorText.slice(0, 180)}`);
        return;
      }

      const payload = (await response.json()) as MarketDataDebugSnapshot;
      if (!payload || !Array.isArray(payload.stocks)) {
        setReloadMessage("재검사 실패: 응답 형식을 확인할 수 없습니다.");
        return;
      }

      setSnapshot(payload);
      setReloadMessage("KIS 연결 재검사가 완료되었습니다.");
    } catch (error) {
      setReloadMessage(error instanceof Error ? `재검사 실패: ${error.message}` : "재검사 실패");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      <section className="rounded-lg border border-line bg-white p-5 shadow-soft dark:border-dark-line dark:bg-dark-panel">
        ...
        {reloadMessage ? (
          <p className="mt-2 text-xs font-semibold text-slate-600 dark:text-slate-300">{reloadMessage}</p>
        ) : null}
